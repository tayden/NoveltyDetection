...
kwargs = {'num_workers': 2, 'pin_memory': True} if use_cuda else {}
cifar20_loader = torch.utils.data.DataLoader(
    datasets.ImageFolder(os.path.join(DATA_DIR, 'cifar20/test'), transform=transform),
    batch_size=BATCH_SIZE, shuffle=True, **kwargs)

num_cifar20 = len(cifar20_loader.dataset)
...
cifar20_odin_maximums = get_max_odin_outputs(
    cifar20_loader, device, temp=temp, noiseMagnitude=eps)


# In[25]:


labels = [1] * num_inliers + [0] * num_cifar20
predictions = cifar_model_maximums + cifar20_model_maximums
predictions_odin = cifar_odin_maximums + cifar20_odin_maximums

stats = get_summary_statistics(predictions, labels)
df.loc['cifar20'] = pd.Series(stats)

stats_odin = get_summary_statistics(predictions_odin, labels)
df_odin.loc['cifar20'] = pd.Series(stats_odin)
...
df.to_pickle('./results/cifar80_wrn28_10_liang2018.pkl')
df_odin.to_pickle('./results/cifar80_wrn28_10_odin_liang2018.pkl')
